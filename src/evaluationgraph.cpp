...
{
    const QStringList scaleTexts = {"4k", "2k", "0", "2k", "4k"};
    ...
    QRect textRect(0, 0, margin.left() - 2, 12);
    ...
    if (!_scores.isEmpty()) {
        ...
        for (int i = 0; i < _scores.count(); i++) {
            int eval = _scores[i];
            int x = sx + i * dx;
            int y = qBound(grphRect.y(), yzero - eval * grphRect.height() / 2 / 4000, grphRect.y() + grphRect.height());
            //qDebug() << x << y;
            path.lineTo(x, y);
